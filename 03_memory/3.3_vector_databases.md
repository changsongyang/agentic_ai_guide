...
# 运行前提 (Tested with pinecone-client>=3.0.0, Python 3.10+, 验证时间: 2026-03)
...
# 运行前提 (Tested with weaviate-client>=4.4.x, Python 3.10+, 验证时间: 2026-03)

# 必要前提：本地已启动 Weaviate docker 容器
...
try:
    memory_collection = client.collections.create(
        name="Memory",
...
如果你选择自托管方案，下面这个最小 `docker-compose.yml` 示例可以帮助你快速完成本地验证：
...
services:
  weaviate:
    ...
    environment:
      ...
      HUGGINGFACE_APIKEY: 'your-api-key'
```
...
# 运行前提 (Tested with chromadb>=0.4.x, Python 3.10+, 验证时间: 2026-03)
...
# 2. 契约测试用例 (Contract Tests)

def test_vector_store_contract(store: VectorStoreBase):
    """
    只需传入不同的 store 实例（PineconeStore, ChromaStore 等），
    即可自动验证其核心功能是否完整工作。
    """
    ...
    assert len(res1) == 1
    assert "苹果" in res1[0]["text"]

    # 场景 2：元数据过滤
    res2 = store.search(query="红色的", top_k=5, filter_dict={"color": "blue"})
    # 期望返回蓝色相关对象，尽管 query 在查红色
    assert len(res2) > 0
    assert "天空" in res2[0]["text"]

    print("✅ 契约测试通过！该向量存储实现合格。")
```
