...
- **Adaptive Thinking**（`thinking: {"type": "adaptive"}`）：支持 Claude Opus 4.8、Claude Opus 4.7 和 Sonnet 4.6，由模型自主决定推理深度，无需手动设置预算。
- **Claude Opus 4.8 / 4.7**：不支持 Extended Thinking，仅支持 Adaptive Thinking。Adaptive Thinking 是 Opus 4.8 / 4.7 唯一支持的推理模式，在 API 调用时无法再使用固定的 budget_tokens。
...
def adaptive_thinking_example(complex_task: str):
    ...
    response = client.messages.create(
        ...
        thinking={
            "type": "adaptive",
            # Opus 4.8 默认是 omitted；这里显式要求返回可读的摘要
            "display": "summarized",
        },
