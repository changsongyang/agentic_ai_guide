...
2024 年以后，推理能力开始从“提示词技巧”逐步变成独立的模型产品能力。以 OpenAI 的 o 系列（自 2024 年起形成推理产品线，后续默认主线进一步演进到 GPT-5）、以及 Anthropic 在 Claude 4.x 中提供的 Extended / Adaptive Thinking 为代表，行业趋势变成了：把更多 **测试时计算（test-time compute）** 暴露为可调参数或模式，而不是完全依赖单次前向生成。
