...
def require_file(path: Path) -> None:
    if not path.is_file():
        raise ArtifactVerificationError(f"{path} does not exist or is not a file")
    if path.stat().st_size == 0:
        raise ArtifactVerificationError(f"{path} is empty")
...
def verify_pdf(path: Path, expected_title: str) -> None:
    ...
    with path.open("rb") as source:
        if source.read(5) != b"%PDF-":
            raise ArtifactVerificationError(f"{path} does not have a PDF signature")

    if shutil.which("pdfinfo") is None or shutil.which("pdftotext") is None:
        raise ArtifactVerificationError(
            "pdfinfo and pdftotext are required for PDF title verification"
        )
...
def count_summary_mermaid_blocks(source_root: Path) -> int:
    ...
    for match in re.finditer(
        ...
        if source_root not in path.parents and path != source_root:
            raise ArtifactVerificationError(
                f"SUMMARY entry escapes source root: {relative}"
            )
        if path in seen:
            continue
        require_file(path)
...
def verify_html(
    ...
    *,
    ...
    if any("<svg" not in figure.lower() for figure in figures):
        raise ArtifactVerificationError(
            f"{path} contains a Mermaid figure without an inline SVG"
        )
...
def write_checksums(paths: list[Path], destination: Path) -> None:
    ...
    for path in sorted(paths, key=lambda item: item.name):
        ...
        if path.parent.resolve() != destination.parent.resolve():
            raise ArtifactVerificationError(
                f"{path} must be beside checksum manifest {destination}"
            )
        if path.name in names:
            raise ArtifactVerificationError(f"duplicate artifact name: {path.name}")
        names.add(path.name)
        lines.append(f"{sha256_file(path)}  {path.name}\n")
    if not lines:
        raise ArtifactVerificationError("cannot write an empty checksum manifest")
...
def verify_checksums(manifest: Path) -> None:
    ...
    if not entries:
        raise ArtifactVerificationError(f"{manifest} is empty")
    names: set[str] = set()
    for line_number, line in enumerate(entries, 1):
        match = re.fullmatch(r"([0-9a-f]{64})  ([^/\\]+)", line)
        if match is None:
            raise ArtifactVerificationError(
                f"{manifest}:{line_number}: malformed SHA-256 entry"
            )
