...
class AgentWithDisclaimer:
    def __init__(self):
        self.disclaimer = "本智能体提供的建议仅供参考，不构成专业意见。重要决策请咨询人类专业人士。"
...
class PrivacyCompliantAgent:
    def process_user_data(self, data: UserData):
        ...
        self.log_processing_purpose(data_id=data.id, purpose="客户咨询服务", legal_basis="用户同意")
    ...
    def audit_decision(self, decision: Decision, context: Context):
        ...
        for attr in protected_attributes:
            if self._is_influenced_by(decision, attr, context):
                self.flag_for_review(decision, f"受 {attr} 影响")
```
...
class WelfareFirstAgent:
    async def should_proceed(self, action: Action) -> bool:
        if self._harms_humans(action) or self._insufficient_human_confirmation(action):
            return False
        return self._safety_over_efficiency(action)
```
...
### 11.3.11 区块链治理框架与 AI 特定法律实体

随着链上自主智能体（on-chain autonomous agents）的出现，传统的 AI 治理框架（如欧盟 AI 法案）暴露出缺陷：它们假设存在可识别的责任方，但公链智能体具有去中心化执行、不可逆转和跨国界等特性，使得责任链路极其复杂。
...
**关键提案**：
- **ERC-6551**（代币绑定账户）：允许智能体持有和管理资产，所有交易记录在链上，提供完整审计线索
- **ERC-8004**（身份与信誉注册表）：为智能体提供跨链身份验证和信誉评分
- **AgentRegistry**：允许智能体在特定域内自动发现、验证和交互其他智能体

这些框架试图在智能体经济自主性和问责机制之间找到平衡：链上不可篡改记录保证审计，去中心化仲裁与强制保险池承担责任，动态风险评分防止权限膨胀。
