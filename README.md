...
模型可用性、API、协议和基准等高波动内容统一收录在[快变事实核验表](12_appendix/12.5_volatile_facts.md)，并按 30 天有效期复核。正文出现冲突时，以该表引用的最新官方来源和冲突状态为准。

## 高管决策路线

这条五站路线把技术自主性转换为可审计的投资与治理决策；每一站都有明确输入和输出，可由测试验证链接与顺序。

1. [定义可接受的自主度与升级边界](09_agentops/9.9_autonomy_metrics.md)：确定授权层级、人工接管点和成功指标。
2. [建立成本、性能与 ROI 基线](09_agentops/9.4_optimization.md)：用真实采样评估单位经济性，而不是用演示效果外推。
3. [落实企业治理与运行责任](09_agentops/9.5_enterprise.md)：把权限、审计、合规和事故响应绑定到负责人。
4. [设计可理解、可撤销的人机体验](04_tools/4.7_agentic_ux.md)：验证用户是否看得懂、控得住并能安全接管。
5. [审查对齐与长期外部性](11_future/11.2_alignment.md)：在扩大自治范围前复核目标偏移、滥用与系统性风险。
