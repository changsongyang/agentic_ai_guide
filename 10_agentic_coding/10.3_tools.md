...
|------|-------------------------------|------------------------|
| **隐私**| ✅**显著降低数据出域风险**，但仍需主机、权限、审计与供应链防护 | ❌ 代码需上传云端 |
...
不同平台的安装方式略有差异，建议按官方安装流程完成安装。安装完成后，至少应验证命令入口可用：
...
Claude Code 使用 `CLAUDE.md` 作为项目级持久指令文件，可以把仓库约束、技术栈和工作约定写进去：
...
Codex CLI 当前把“是否需要审批”和“允许多大执行权限”拆成两个维度：

| 维度 | 常见取值 | 含义 |
|------|-----------|------|
| **审批策略** | `untrusted` / `on-request` / `never` | 决定何时需要人工批准 |
| **沙箱权限** | `read-only` / `workspace-write` / `danger-full-access` | 决定命令可访问的环境边界 |
...
# 低摩擦自动执行（等价于 on-request + workspace-write）
...
codex -i design.png "按照这个设计稿实现页面"
```
...
Codex 使用 `AGENTS.md` 作为项目级指令文件，用来描述仓库约束、执行边界和工作方式：
...
组合使用多个工具时，关键是明确“谁做什么”，避免重复劳动与上下文相互污染：

- **IDE 内工具**：适合多文件编辑、局部重构、交互式调试
- **CLI 智能体**：适合批处理、仓库级扫描、运行测试与生成变更摘要
- **平台机器人**：适合事件驱动的自动化（Issue→PR、代码审查、定时任务）
