...
#### 10.3.4.5 Effort 分级与 AutoDream

Claude Code 支持 `effort` 参数控制推理深度（`low` / `medium` / `high` / `xhigh`）。其中 `xhigh` 仅在 Opus 4.7 上可用，适合复杂重构、多文件迁移和架构级任务——它会启动多轮规划，覆盖边界条件与跨模块依赖，而 `high` 通常只生成主路径实现。推荐策略是日常使用 `high`，仅在深度推理场景切换到 `xhigh`。

AutoDream 是 Claude Code 的后台记忆整理机制：在会话间隙自动合并相关记忆文件、修剪过期条目、解决冲突记录，并为模糊引用补充时间戳。该机制使长期项目中的 `CLAUDE.md` 和 memory 文件保持一致性，避免上下文漂移。
