...
上一节的 **Think → Act → Observe** 描述了单次迭代内的微观流程：模型如何思考、执行和接收反馈。而从更宏观的工程视角看，一个完整任务的生命周期可以抽象为 **Gather（收集） → Action（行动） → Verify（验证）** 框架——它关注的不是单步循环内部发生了什么，而是每个阶段有哪些可选的工程策略。这个模式在 Anthropic 的 Agent SDK 实现中得到了系统性的阐述。
