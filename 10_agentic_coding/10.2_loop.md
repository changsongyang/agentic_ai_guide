...
#### 10.2.1.2 Gather-Action-Verify 框架

从工程实践的视角，Agent Loop 的三个阶段也可以统一为一个宽泛的设计框架：**Gather（收集） → Action（行动） → Verify（验证）**，这个模式在 Anthropic 的 Agent SDK 实现中得到了系统性的阐述 (《Claude Agent SDK构建智能体》，2025年9月)。
...
| 接口形态 | 停止序列格式 | 工具调用格式 |
|----------|------------|-------------|
| 函数调用接口 | 内置 function calling | JSON Schema |
| XML 标签风格 | `</tool_use>` | XML-like |
...
以下用一个“较大上下文窗口”的示例来说明结构（不同模型窗口大小差异很大）：
...
2. **“上下文恢复”技巧**：很多用户不愿重置对话是因为怕丢失进度，可以先将当前进度（Completed）、下一步计划（Next Steps）和关键决策（Key Decisions）总结到一个 `implementation_plan.md` 或 `handoff_note.md` 文件中，然后在新对话中，只需 `@` 引用这个文件，智能体就能瞬间恢复“工作记忆”，而无需背负之前几百轮对话的垃圾信息（Token 债务）。
