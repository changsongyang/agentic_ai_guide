...
虽然智能体擅长搜索，但通过统一的“项目说明文件”（推荐命名为 `AGENTS.md`，见 [附录 12.3](../12_appendix/12.3_agents_md.md)）提供项目级的“机器可读说明书”，能减少幻觉与不必要的上下文检索时间。这是智能体编程的“地图与宪法”。

> **重要**：`AGENTS.md` 不应仅仅是文档文本，它更应该被当作 **最小权限原则 (Least Privilege)** 的配置中心。例如在其中声明允许调用的 MCP 服务器列表与允许读写的目录，在底层执行沙箱中读取此文件进行强拦截。
...
#### 10.5.5.3 案例：OpenAI 的 HumanEval 基座
