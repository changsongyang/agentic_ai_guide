...
> **框架生态更新（2026）**：Microsoft 于 2025 年 10 月合并 AutoGen 与 Semantic Kernel，推出统一的 **Microsoft Agent Framework**（RC 版本于 2026 年 2 月发布，GA 预计 Q1 2026）。原 AutoGen 开源项目已转入维护模式，仅接收安全补丁。AutoGen 的原创建者则在 2025 年 11 月基于社区治理模式推出 **AG2**，保留熟悉的 AutoGen 0.2 架构并持续迭代。
...
```python
# AG2 示例（推荐）：用"群聊编排"构建自动编程团队
# AG2 是社区维护的 AutoGen 分支，保留 AutoGen 0.2 API 并持续迭代
...
group_chat = GroupChat(agents=[executor, pm, coder], max_round=12)
manager = GroupChatManager(groupchat=group_chat)

# 启动群聊编排
pm.initiate_chat(manager, message="我想做一个简单的贪吃蛇游戏。")

# 循环：pm -> coder -> executor(运行/报错) -> coder(修复) -> ... -> 产出可运行程序
```

**运行结果**：PM 会先列出功能点，Coder 接着写代码，Executor 尝试运行，如果报错 Coder 会自动修。你会看到这三个”人”在屏幕上快速刷屏对话，最后直接在目录下生成一个可运行的 `snake.py`。

> **扩展阅读**：如需使用 Microsoft 官方支持的框架，可考虑 **Microsoft Agent Framework**（新），它统一了原 Semantic Kernel 与 AutoGen 能力，提供更先进的事件驱动架构。如果项目已采用旧版 AutoGen（v0.2）且无 breaking change 耐受度，AG2 是更好的延续选择。
...
```python
# CrewAI 示例（v1.12.2, March 2026）：用"角色 + 任务流"构建市场调研团队
...
| 维度 | 对话编排范式 | 角色任务编排范式 |
| :--- | :--- | :--- |
| **代表框架** | AG2（维护中）/ Microsoft Agent Framework（新） | CrewAI（v1.12+，活跃） |
...
- 一类框架更偏”执行闭环”（写代码/跑代码/修复），典型如 AG2（社区维护）或 Microsoft Agent Framework（新方向）。
- 一类框架更偏”角色流程”（研究→写作→审阅→交付），典型如 CrewAI（v1.12.2, 2026 年 3 月）。

**框架选择指南**：
- 如果项目已使用 AutoGen 0.2 且需要稳定维护，迁移至 **AG2**（社区驱动）是自然选择。
- 如果新项目需要最新的事件驱动架构与 Microsoft 官方支持，考虑 **Microsoft Agent Framework**（RC → GA）。
- 如果关注快速原型与明确的流程控制，**CrewAI** 提供完善的任务流与角色管理。
