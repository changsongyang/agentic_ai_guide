...
- **基础依赖**：操作系统版本、基座语言环境（如 Python 3.10）
- **模型服务**：模型版本（如 GPT-4-turbo-1106）、部署形态（公有云 SaaS / 私有化 vLLM）
...
**优化策略**（包括提示词缓存、上下文压缩、计划缓存等）详见[第 9.3 节](../09_agentops/9.3_optimization.md)。本节重点在于**框架间的相对 Token 膨胀系数对比**，以便在选型时准确评估不同框架的开销特性。
...
class AgentBenchmark:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print(f"[{self.name}] Latency: {self.metrics['latency_ms']:.2f}ms | Peak RAM: {self.metrics['peak_memory_mb']:.2f}MB")
...
基于真实场景评测（单工具调用 + 状态序列化，N=100 次运行），以下提供主流框架的近似性能指标。**注意**：实际数值因任务类型、网络延迟、模型版本而异，建议在自己的环境中重复评测：

| 框架 | 端到端延迟 (P95, ms) | 峰值内存占用 (MB) | Token 膨胀系数 | 并发能力评级 |
|------|------------------|-----------------|----------------|-----------|
| LangGraph | ~400–500 | 80–120 | 1.4–1.6× | 高（异步原生） |
| LlamaIndex Workflows | ~380–450 | 70–100 | 1.3–1.5× | 高（事件驱动） |
| CrewAI | ~600–800 | 100–150 | 1.8–2.2× | 中（每轮完整 LLM 调用） |
| Haystack Pipeline | ~350–420 | 60–90 | 1.2–1.4× | 高（轻量） |

**说明**：
- 延迟包含工具调用往返与状态管理开销，不含模型推理。
- CrewAI 延迟较高因其多智能体协作需多轮对话（每轮都调用 LLM）。
- Token 膨胀系数是框架隐式添加的协议/提示词相对于用户业务逻辑的比值。
- 并发能力受工具限流、异步支持、批处理能力影响。
...
```mermaid
quadrantChart
    title Framework Selection Matrix (框架选型矩阵)
    x-axis Low Latency --> High Latency
    y-axis Lightweight/Edge --> Heavy/Enterprise
    quadrant-1 Complex Enterprise Flows (复杂企业流)
    quadrant-2 All-in-One (全能型)
    quadrant-3 Ultra-Fast Interaction (极速交互)
    quadrant-4 Offline Batch Processing (离线批处理)
    Light Orchestration: [0.15, 0.25]
    Graph/State Machine: [0.35, 0.6]
    Conversational Multi-Agent: [0.6, 0.7]
    SOP/Enterprise Flow: [0.75, 0.8]
```
