...
> **重要变化（2026）**：Microsoft 于 2025 年 10 月推出 **Microsoft Agent Framework**，正式合并 Semantic Kernel 与 AutoGen 为统一平台。Semantic Kernel v1.x 将继续维护至少 1 年，但新特性优先在 Microsoft Agent Framework 上开发。本节以 Semantic Kernel 1.41.1（2026 年 3 月）为例，同时介绍 Microsoft Agent Framework 的过渡路径。
...
- **Semantic Kernel 1.x 时代**（当前维护）：成熟稳定，适合生产环保。
...
阅读时重点关注”注册服务”与”自动函数调用配置”这两段，它们决定了旧系统能力如何被模型稳定编排。

```python

# Semantic Kernel 1.41.1 示例：初始化 Kernel、注册服务与插件，并启用自动函数调用

import os
from semantic_kernel import Kernel
from semantic_kernel.connectors.ai.open_ai import OpenAIChatCompletion
from semantic_kernel.connectors.ai.prompt_execution_settings import FunctionChoiceBehavior, PromptExecutionSettings
...
async def main():
    ...
    # 2. 配置 AI 服务（注意：API 已从 add_model_service 改为 add_service）
...
> **API 变更说明**：
> - `add_model_service()` 已改为 `add_service()`（Semantic Kernel 1.41+）
...
Microsoft Agent Framework（RC 版本）在 Semantic Kernel 基础上增强了：
