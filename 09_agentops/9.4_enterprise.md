...
关于 Agent 安全威胁的完整分类（提示词注入 vs 越狱、供应链攻击等）和通用防御策略，详见[第 11.1 节](../11_future/11.1_security.md)。本节重点关注**企业级部署中的工程实现与运维观点**。

#### 多层防御架构速览

| 防御层级 | 关键技术 | 主要作用 | 详见章节 |
|---------|--------|--------|--------|
| Layer 1-3 | Prompt 工程、护栏、应用鉴权 | 在应用逻辑层防止越权与不当调用 | 9.4.2、11.1.2 |
| Layer 4 | 结构化输出约束 | 确保模型输出符合 Schema，防止注入导致的格式污染 | 9.4.2（下文） |
| Layer 5 | 基础设施沙箱 | 隔离代码执行环境，防止恶意代码逃逸 | 9.4.2（下文） |
| Layer 6 | 侧车代理安全联网 | 对外网访问的密钥与域名白名单管理 | 9.4.2（下文） |
...
侧车代理模式已被 OpenAI Responses API 等生产级智能体平台采用。其核心优势在于将安全策略从应用逻辑中解耦，使开发者可以独立升级安全规则而无需修改智能体代码。
...
阶段越靠后，越需要把“权限、审计、回滚、成本与故障恢复”做成平台能力，而不是靠应用侧临时补丁。这些平台级能力——从系统指令预设、工具调用管理到生命周期钩子——共同构成了所谓的 **智能体运行壳（Agent Harness）**：一个包裹在模型外围、专门管理长周期任务的操作系统级基础设施层（详见 [11.4 节](../11_future/11.4_agi_path.md)）。

---

**下一节**: [9.5 故障模式与韧性设计](9.5_failures.md)
