# 第五章 多智能体协作模式

俗话说：“三个臭皮匠，顶个诸葛亮”。单个智能体的能力受限于模型能力与上下文窗口，而多智能体系统通过专业分工和协作，能够涌现出处理复杂系统的能力。本章将带你进入“AI 团队”的世界，探索不同的协作架构和组织模式。

## 章节导读

- **[5.1 协作架构：层级、扁平与动态](5.1_architectures.md)**
  - 就像公司有组织架构，多智能体系统也有不同的组织模式：**顺序式**、**层级式** 与 **网状**。本节将解析这些协作范式在工程实践中的典型落地方式。

- **[5.2 角色分工与 SOP 流程编排](5.2_roles_sop.md)**
  - 每个智能体都有自己的”人设”：角色定义、目标与背景故事。学习如何设计标准作业程序来协调多个智能体的工作流程。

- **[5.3 动态组队与自适应编排](5.3_dynamic_teaming.md)**
  - 静态的团队配置无法应对复杂多变的任务。探讨如何让智能体根据任务需求动态组队，实现自适应的任务编排。

- **[5.4 人机协作](5.4_hitl.md)**
  - 完全自主的智能体并不总是最佳选择。学习如何在关键节点引入人类监督，实现人机协作的最佳平衡。

- **[本章小结](summary.md)**

---

## 核心概念预览

```mermaid
graph TD
    %% Agentic Design System
    classDef agent fill:#e6f7ff,stroke:#1890ff,stroke-width:2px;
    classDef tool fill:#f6ffed,stroke:#52c41a,stroke-width:2px;
    classDef memory fill:#fff0f6,stroke:#eb2f96,stroke-width:2px;

    subgraph MAS [多智能体协作系统]
        Supervisor[监管智能体 Supervisor] -->|协调| W1[Worker1]
        Supervisor -->|分派任务| Memory[Shared Memory]

        W1 --> Memory
        W2[Worker2] -->|交接| W3[Worker3]

        Memory <--> W2
        Memory <--> W3
    end

    class Supervisor agent;
    class W1,W2,W3 tool;
    class Memory memory;
```

图 5-0：多智能体协作系统示意图

下一章将探讨智能体之间如何进行有效的通信，以及如何模拟真实的社会行为。

---

**下一节**: [5.1 协作架构：层级、扁平与动态](5.1_architectures.md)
